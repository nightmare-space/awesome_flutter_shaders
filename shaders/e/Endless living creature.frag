// --- Migrate Log ---
// 把 motion_frames 从 float 改为 int，使用 int 循环以兼容 SkSL；显式初始化 fragColor 为 vec4(0.0)；添加 common header 与 main_shadertoy include
// Changed motion_frames from float to int and replaced float loop with int loop for SkSL compatibility; explicitly initialize fragColor as vec4(0.0); added common header and main include
//
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    float total = 0.0;
    fragColor = vec4(0.0);
    for (int idx = motion_frames; idx > 0; --idx) {
        float index = float(idx);
        float dither = random(ray.xy + fract(iTime + index));
        float time = iTime * speed + (dither + index) / 10.0 / float(motion_frames);
        fragColor += vec4(raymarch(eye, ray, time, total)) / float(motion_frames);
    }
